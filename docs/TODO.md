...
> **Estado:** Este documento es un **roadmap/TODO**, no una especificación técnica implementada. Representa la visión de producto y las historias de usuario planned. Para la especificación técnica actual, ver [`agent-schema.md`](./agent-schema.md) y los manifiestos en `docs/manifests/`.
...
_Responsable de la persistencia, configuración, facturación y panel de control._
...
_Responsable del procesamiento de audio, latencia y flujos de IA._
...
- **[ ] HU-5.3: Análisis de Sentimiento en Tiempo Real**
    - **Como** Sistema.
    - **Quiero** detectar la carga emocional del usuario durante la llamada.
    - **CA:** Implementar procesador de análisis de sentimiento (VADER o modelo de HuggingFace) en el pipeline. Si el cliente está muy frustrado, activar automáticamente la herramienta `transferir_a_agente`.
- **[ ] HU-5.4: Gestión de Estados Visuales (RTVI Events)**
    - **Como** Frontend / Widget.
    - **Quiero** recibir notificaciones activas sobre lo que está haciendo el bot.
    - **CA:** Emitir eventos RTVI (`bot-thinking`, `bot-listening`, `tool-executing`) para que el widget web actualice su interfaz.
...
- **[ ] HU-6.2: Manejo de Errores Avanzado y Resiliencia**
    - **Como** Sistema.
    - **Quiero** lógica de reintento y mensajes empáticos ante fallos de API.
    - **CA:** Capturar excepciones en `AgentTools` y devolver un mensaje estructurado al LLM para que el agente explique el problema (ej. "El sistema está lento, deme un momento adicional..."). Evita silencios o respuestas genéricas cuando un servicio externo falla.

---

## Fase 3: Operaciones SaaS Pro y Calidad de Servicio

_Características premium y herramientas de observabilidad._
...
_Foco: Transparencia y confianza en la IA._
...
- **[ ] HU-8.2: Visualizador Waterfall de Latencia (Modo Debug)**
    ...
    - **Quiero** ver el desglose en ms de cada fase: _STT -> LLM (TTFT) -> TTS_.
...
_Características para despliegues corporativos, banca y alta disponibilidad._
...
- **[ ] HU-12.2: Ambient Noise (Ruido de Fondo)**
    ...
    - **Quiero** reproducir ruidos de ambiente en bucle durante toda la llamada para aumentar el realismo.
    - **CA:** Configuración vía `runtime_profiles.behavior.ambient_sound`.

---

## Fase 5: Estandarización de Plataforma y Arquitectura Core

_Unificación de los diferentes tipos de agentes (MVP, Pipeline, Flow) bajo un mismo estándar arquitectónico._

### _Nota:_ El esquema base unificado ya fue consolidado en `docs/agent_unified_manifest.json`.
...
_Capa comercial, integraciones de cara al cliente y herramientas orientadas a potenciar la adopción SaaS._
...
- **[ ] HU-17.1: Motor de Clonación Multi-Proveedor (Voice Cloning Sync)**
    ...
    - **Implementación Lógica Multi-Proveedor (El 'Cómo'):**
        ...
        2. **Sync Asíncrono Concurrent:** Cuando un usuario de un Tenant sube su MP3 (`POST /api/voices/clone`), Laravel disparará un Job que realiza el API Request _en paralelo_ a todos los proveedores configurados/habilitados para entrenar la voz en cada uno de ellos simultáneamente.
