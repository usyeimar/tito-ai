...
return [
    ...
    /*
     ...
     |--------------------------------------------------------------------------
     |
     | Lifespan of impersonation tokens in seconds. Tokens are single-use
     | and are purged after this TTL elapses.
     |
     */
    'impersonation_ttl' => (int) env('TENANCY_IMPERSONATION_TTL', 60),
    ...
    'database' => [
        ...
        /**
         * The connections that should be switched for tenants.
         */
        'connections' => [
    ...
    'migration_parameters' => [
        ...
        '--path' => (function (): array {
            $root = database_path('migrations/tenant');
            if (! is_dir($root)) {
                return [];
            }
